...
public class DetailController implements Initializable {
    ...
    private void tampilkanRekomendasiBuku(List<Buku> bukuRekomendasi) {
        ...
        if (bukuRekomendasi != null && !bukuRekomendasi.isEmpty()) {
            int column = 0;
            int row = 1;
            List<Buku> bukuAdded = new ArrayList<>(); // Menyimpan buku yang sudah ditambahkan

            // Menambahkan buku-buku dengan genre yang sama
            for (int i = 0; i < bukuRekomendasi.size(); i++) {
                Buku buku = bukuRekomendasi.get(i);
                boolean isDuplicate = false;

                // Memeriksa apakah buku sudah ditambahkan sebelumnya
                for (Buku addedBuku : bukuAdded) {
                    if (buku.equals(addedBuku)) {
                        isDuplicate = true;
                        break;
                    }
                }
                ...
                if (!isDuplicate) {
                    ...
                    try {
                        ...
                        // Menambahkan buku ke daftar yang sudah ditambahkan
                        bukuAdded.add(buku);
                    } catch (IOException e) {
                        e.printStackTrace();
    ...
    private void tampilkanBukuSamaGenre(List<Buku> bukuSamaGenre) {
        ...
        if (bukuSamaGenre != null && !bukuSamaGenre.isEmpty()) {
            ...
            // Menambahkan buku-buku dengan genre yang sama
            for (int i = 0; i < bukuSamaGenre.size(); i++) {
                Buku buku = bukuSamaGenre.get(i);
                boolean isDuplicate = false;

                // Memeriksa apakah buku sudah ada sebelumnya
                for (int j = i + 1; j < bukuSamaGenre.size(); j++) {
                    if (buku.equals(bukuSamaGenre.get(j))) {
                        isDuplicate = true;
                        break;
                    }
                }
    ...
    private void tampilkanBukuSamaPenulis(List<Buku> bukuSamaPenulis) {
        ...
        if (bukuSamaPenulis != null && !bukuSamaPenulis.isEmpty()) {
            ...
            for (int i = 0; i < bukuSamaPenulis.size(); i++) {
                Buku buku = bukuSamaPenulis.get(i);
                boolean isDuplicate = false;

                // Memeriksa apakah buku sudah ditambahkan sebelumnya
                for (int j = i + 1; j < bukuSamaPenulis.size(); j++) {
                    if (buku.equals(bukuSamaPenulis.get(j))) {
                        isDuplicate = true;
                        break;
                    }
                }
